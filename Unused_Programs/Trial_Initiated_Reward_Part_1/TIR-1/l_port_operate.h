...
void activate_left_led() {
     ...
     if (solenoid_active_L == false) {
       ...
       // (PORT LIGHT ON) Turn on Left LED if it is a valid trial
         ...
         if ((valid_trial_window) && (led_state_L == HIGH) && (poke_in_L) && (valid_input_L)) {
...
void activate_left_sol () {
    ...
    if (led_state_L == LOW) {
        ...
        if ((reward_window) && (reward_type == 1)) {
            ...
            Serial.print(F("7271:"));     // Timestamp for Valid Solenoid On
          ...
          if ((sol_state_L == HIGH) && (millis() - solenoid_on_time > solenoid_on_duration)) {
             ...
             solenoid_active_L = false;   // can now activate the leds
             sol_state_L = LOW;
             digitalWrite(port_solenoid_L, sol_state_L);
             ...
             Serial.print(F("7270:"));   // Timestamp for Valid Solenoid Off
