     ...
     if (solenoid_active_M == false) {
       ...
       // (PORT LIGHT ON) Turn on Left LED if it is a valid trial
         ...
         if ((valid_response_window) && (led_state_L == HIGH) && (poke_in_L) && (left_port_counter > 0)) {
           ...
           FC_delay_start_time = millis(); // record when the reward delay started  
         ...
         // (INVALID_POKE - LIGHT OFF) --> poke in the unlit right or middle ports --> turn off the left LED and and start reward window (type 2 = no reward)
...
void activate_left_sol () {  // controls Reward Delay & Reward Window functionality (delay in reward delivery + reward cue presentation + reward delivery) 
