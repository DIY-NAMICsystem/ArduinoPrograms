...
void begin_new_trial() {  // controls initiation window functionality + runs at the beginning of each new trial 
  ...
  if (initiate_new_trial){
    ...
    // this if statement allows the experimenter to introduce a delay between the initiation and trial windows 
    ...
    if (initiate_trial_delay){
       ...
       // (trial_delay WINDOW is 1 ms (practically 0ms) in the original DIY-NAMIC delay discounting paradigm)
