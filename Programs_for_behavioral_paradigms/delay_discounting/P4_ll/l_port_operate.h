...
/* 
 *  Function: 
 *  Operate the left LED during valid response window --> FORCED CHOICE TRIAL ON THE LEFT PORT
    ...
    if (led_state_L == LOW) {
        ...
        if ((reward_window) && (reward_type == 1)) {
            ...
            reward_type = 0; // toggle back to default value
			      int i = 1;       // will count the number of drops given (3 drops will be given)
           ...
           //* solenoid on time = reward cue start time! *//
            ...
            while (i <= 3) {  // changing 3 here can be used to change the number of reward drops dispensed 
             ...
             Serial.print(F("82270:"));   // Timestamp for FC Valid Solenoid Off --> can be used to count the number of drops given 
           ...
           if ((reward_cue_window) && (led_state_M == HIGH) && (millis() - solenoid_on_time > reward_cue_duration) ) {
             ...
             // POKES during the REWARD Window (Force Choice Event Code)
