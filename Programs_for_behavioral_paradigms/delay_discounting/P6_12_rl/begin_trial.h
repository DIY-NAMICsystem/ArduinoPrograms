...
void begin_new_trial() {  // controls initiation window functionality + runs at the beginning of each new trial 
  ...
  if (initiate_new_trial){
    ...
    // Also when trial_delay window didn't start --> (delay window is 1 ms in the original DIY-NAMIC delay discounting paradigm)  
    ...
    if ((initiate_trial_delay == false) && (valid_response_window)) {
       ...
       // Initiate trial_delay Window (1 ms (practically 0) in the original DIY-NAMIC delay discounting paradigm) 
       initiate_trial_delay = true;

       }
       
    // this if statement allows the experimenter to introduce a delay between the initiation and response windows 
    // 1 ms in the original DIY-NAMIC delay discounting paradigm --> change trial_delay_duration in globals_time.h to adjust delay 
