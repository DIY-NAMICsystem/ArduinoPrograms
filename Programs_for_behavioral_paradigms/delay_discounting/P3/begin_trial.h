...
void begin_new_trial() { // controls initiation window functionality + runs at the beginning of each new trial 
  ...
  // Initiate New Trial is toggled on AFTER iti duration expires (from M port operate.h) OR when the program first starts 
  ...
  if (initiate_new_trial){
    ...
    if ((initiate_trial_delay == false) && (initiation_window == false) && (valid_response_window == false)){
       ...
       // Turn ON All three LEDs (can be used to debug the code) 
       ...
       initiation_window_start_time = millis();  // time stamp for trial initiation 
    ...
    // Valid response window toggles TRUE if poke in blinking middle port (see IR.h)
    ...
    // introduces a delay period between trial initiation and the presentation of the response cue 
    ...
    if (initiate_trial_delay){
      ...
      // (trial_delay PERIOD EXPIRES) (trial_delay WINDOW ENDS) 
      // (trial_delay WINDOW will be 1 ms (practically 0ms) in the original DIY-NAMIC delay discounting paradigm)
       if ((millis() - initiation_window_end_time) >= trial_delay_duration) {
         ...
         // # of poke counts during trial_delay window (between trial initiation and port activation) --> uncomment if delay period > 1 ms 
         ...
         // FOR DEBUGGING PURPOSES: prints out the selected port 
