  ...
  if (read_gate_M != ir_previous_state_M) {  // registers middle port IR beam breaks
    if (read_gate_M == LOW) {
      ...
      if (initiation_window){ 
        ...
        // Prints Out number of invalid pokes made when the middle port was blinking 
        // (number of pokes in left/right ports before initiation in the middle port)
        // (# of mid port is always going to be 1 (ONE)): a poke in the middle port is a correct response and will initiate the trial window 
        //    --> since trial needs to be initiated to print out below statements

        // CHANGED HERE (JHL) (2/14/20) --> automatically subtract one from mid_port_counter to account for poke to initiate trial

        // # of poke counts during the initiation window 
        ...
        valid_response_window = true; // start of valid trial (where the animal will respond (i.e poke) to get reward)

        // reset all counters once valid_response_window starts (is true)
        // allows to count the "valid" pokes after trial initiation (during valid response window)
